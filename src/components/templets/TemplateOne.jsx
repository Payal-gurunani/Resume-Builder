...
export default function TemplateOne({ resumeData }) {
  ...
  return (
    <Box maxWidth="lg" mx="auto" my={5} boxShadow={4} display="flex" flexDirection={{ xs: 'column', md: 'row' }}>
      {/* LEFT COLUMN */}
      <Box width={{ xs: '100%', md: '35%' }} bgcolor="background.paper" p={3}>
        ...
        <Stack spacing={1}>
          ...
          {personalInfo.linkedin && (
            <ContactLine
              icon={<FaLinkedin />}
              text={<Link href={personalInfo.linkedin} rel="noopener noreferrer" target="_blank">{personalInfo.linkedin}</Link>}
            />
          )}
          {personalInfo.github && (
            <ContactLine
              icon={<FaGithub />}
              text={<Link href={personalInfo.github} rel="noopener noreferrer" target="_blank">{personalInfo.github}</Link>}
            />
          )}
        ...
        {education.map((edu, i) => (
          <Box key={i} mb={2}>
            ...
            <Typography variant="caption" color="text.secondary">{edu.year}</Typography>
          </Box>
        ))}
      ...
      {/* RIGHT COLUMN */}
      <Box width={{ xs: '100%', md: '65%' }} bgcolor="background.default" p={3}>
        {objective && (
          <>
            <SectionTitle>Summary</SectionTitle>
            <Typography variant="body2" paragraph>{objective}</Typography>
          </>
        )}
